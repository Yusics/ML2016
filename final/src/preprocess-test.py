...
class Model(object):
	...
	def check(self):
		print "len = ", len(self.id), len(self.title), len(self.content)
		#print self.title[0:5][:10]
		for i in np.arange(0, 5, 1):
			print "ID = ", model.id[i]
			print "Title = ", model.title[i][:20]
			print "Content = ", model.content[i][:20]
	...
	def save(self):
		pickle.dump(self.id, open("model/" + self.output + "-id", "wb"))
		pickle.dump(self.title, open("model/" + self.output + "-title", "wb"))
		pickle.dump(self.content, open("model/" + self.output + "-content", "wb"))
		#pickle.dump(self.tags, open("model/tags-" + self.output, "wb"))
